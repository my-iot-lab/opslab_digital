...
namespace Emes.Application.Utils;
...
public static class Object2ValueHelper
{
    ...
    public static T? To<T>(object obj)
    {
        ...
        // Newtonsoft 框架非数组与原始值一样
        // 注：对应整数类型，Newtonsoft 转换为 long, 而 System.Text.Json 为 int32；
        //  对应浮点类型，Newtonsoft 转换为 decimal, 而 System.Text.Json 为 double；
    ...
    public static T?[] ToArray<T>(object obj)
    {
        ...
        {
            return JsonObject2Array<T>(jArray);
        }
    ...
    /// <summary>
    /// 将基于 System.Text.Json 框架的 JSON 反序列化的对象转换为指定类型的基元对象数组。
    /// </summary>
    /// <typeparam name="T">要转换的基元类型</typeparam>
    ...
    /// <summary>
    /// 将基于 Newtonsoft 框架的 JSON 反序列化的对象转换为指定类型的基元对象数组。
    /// </summary>
    /// <typeparam name="T">要转换的基元类型</typeparam>
    /// <param name="jsonElement">Json 元素</param>
    /// <returns></returns>
    private static T?[] JsonObject2Array<T>(JArray jsonElement)
    {
        return jsonElement.Values<T>().ToArray();
    }

    /// <summary>
    /// 将基于 System.Text.Json 框架的 JSON 反序列化的对象转换为指定类型的基元对象。
    /// </summary>
    /// <typeparam name="T">要转换的基元类型</typeparam>
    /// <param name="jsonElement">Json 元素</param>
    /// <returns></returns>
